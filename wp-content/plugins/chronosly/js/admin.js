...
jQuery(document).ready(function($){
    ...
    //escrol del mini paint

	$(window).scroll(function(){
		if($(window).scrollTop() > $(".dad_box").offset().top && style_left){
				//begin to scroll
				//console.log(($(".ev-styling").offset().top + $(".ev-styling").outerHeight())+"<"+ ($(".chronosly").offset().top + $(".chronosly").outerHeight()))
				if( ($(".ev-styling").offset().top + $(".ev-styling").outerHeight()) < ($(".dad_box").offset().top + $(".dad_box").outerHeight()) || ($(window).scrollTop() < ($(".dad_box").offset().top + $(".dad_box").outerHeight() - $(".ev-styling").outerHeight())) ) {$(".ev-styling").css("position","fixed").css("top","50px").css("left", style_left).css("margin-top", 0);}
				else {
                    $(".ev-styling").css("position","relative").css("top", 0).css("left",0).css("margin-top",  ($(".dad_box").outerHeight()-$(".ev-styling").outerHeight()));

                }
			}
			else if(style_left){
				//lock it back into place
				$(".ev-styling").css("position","relative").css("top",0).css("left",0);
		}
...
//            template: templ,
		...
		$.post( ajaxurl, args, function( data ) {
          ...
          $(".main_box").attr("id", vista);//cambiamos el id del template
            ...
            $('#tdad_box .chronosly').bind("DOMSubtreeModified",function(){

                if(guardamos != "chbd")  tempbase = guardamos;
                guardamos = "chbd";//le definimos que se guarda la bd en el template

              $(".tdad_select option[value='template_edited']").attr('selected', 'selected');
    ...
    //solo guardamos en bd cuando esta modificando el template
	$('#tdad_box .chronosly').bind("DOMSubtreeModified",function(){
...
function load_template(){
    ...
    jQuery.post( ajaxurl, args, function( data ) {
        ...
        if(jQuery("#chronosly_chronosly_vars_section input[name='featured']:checked").length ||
           ...
           jQuery("#chronosly_place_chronosly_places_vars_section input[name='featured']:checked").length)  jQuery(".show_featured").click();
        //init_box_controls();
            jQuery('#tdad_box .chronosly').bind("DOMSubtreeModified",function(){

            if(guardamos != "chbd")  tempbase = guardamos;
            guardamos = "chbd";//le definimos que se guarda la bd en el template

          jQuery(".tdad_select option[value='template_edited']").attr('selected', 'selected');
...
function save_all_html_template(){

    var args = {
                action: "chronosly_save_all_html_templates",
...
jQuery(window).load(function(){
    ...
    jQuery("#tdad_box").show();
    jQuery(".dad_controls").show();
});
